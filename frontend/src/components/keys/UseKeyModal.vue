...
import { ref, computed, defineComponent, h, watch, type Component } from 'vue'
import { copyTextToClipboard } from '@/composables/useClipboard'

type GroupPlatform = 'openai' | 'anthropic' | 'gemini' | 'antigravity'
...
const translations: Record<string, string> = {
  'common.close': 'Close',
  'keys.copied': 'Copied',
  'keys.useKeyModal.title': 'Use API Key',
  'keys.useKeyModal.description': 'Add the following environment variables to your terminal profile or run directly in terminal to configure API access.',
  'keys.useKeyModal.copy': 'Copy',
  'keys.useKeyModal.copied': 'Copied',
  'keys.useKeyModal.note': 'These environment variables will be active in the current terminal session. For permanent configuration, add them to ~/.bashrc, ~/.zshrc, or the appropriate configuration file.',
  'keys.useKeyModal.noGroupTitle': 'Please assign a group first',
  'keys.useKeyModal.noGroupDescription': 'This API key has not been assigned to a group. Please click the group column in the key list to assign one before viewing the configuration.',
  'keys.useKeyModal.openai.description': 'Add the following configuration files to your Codex CLI config directory.',
  'keys.useKeyModal.openai.configTomlHint': 'Make sure the following content is at the beginning of the config.toml file',
  'keys.useKeyModal.openai.note': 'Make sure the config directory exists. macOS/Linux users can run mkdir -p ~/.codex to create it.',
  'keys.useKeyModal.openai.noteWindows': 'Press Win+R and enter %userprofile%\\.codex to open the config directory. Create it manually if it does not exist.',
  ...
  'keys.useKeyModal.antigravity.description': 'Configure API access for Antigravity group. Select the configuration method based on your client.',
  'keys.useKeyModal.antigravity.claudeNote': 'These environment variables will be active in the current terminal session. For permanent configuration, add them to ~/.bashrc, ~/.zshrc, or the appropriate configuration file.',
  'keys.useKeyModal.antigravity.geminiNote': 'These environment variables will be active in the current terminal session. For permanent configuration, add them to ~/.bashrc, ~/.zshrc, or the appropriate configuration file.',
  'keys.useKeyModal.gemini.description': 'Add the following environment variables to your terminal profile or run directly in terminal to configure Gemini CLI access.',
  'keys.useKeyModal.gemini.modelComment': 'If you have Gemini 3 access, you can use: gemini-3-pro-preview',
  'keys.useKeyModal.gemini.note': 'These environment variables will be active in the current terminal session. For permanent configuration, add them to ~/.bashrc, ~/.zshrc, or the appropriate configuration file.',
  'keys.useKeyModal.opencode.hint': 'Config path: ~/.config/opencode/opencode.json (or opencode.jsonc), create if not exists. Use default providers (openai/anthropic/google) or custom provider_id. API Key can be configured directly or via /connect command. This is an example, adjust models and options as needed.'
}
...
const BaseDialog = defineComponent({
  ...
  emits: ['close'],
  setup(dialogProps, { slots, emit: dialogEmit }) {
    return () => dialogProps.show
      ? h('div', { class: 'fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm' }, [
          h('section', { class: 'max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-5 shadow-2xl ring-1 ring-black/10 dark:border-dark-700 dark:bg-slate-950' }, [
            h('div', { class: 'mb-4 flex items-start justify-between gap-4' }, [
              h('h2', { class: 'text-lg font-bold text-gray-950 dark:text-white' }, dialogProps.title),
              h('button', { class: 'btn btn-secondary', type: 'button', onClick: () => dialogEmit('close') }, t('common.close'))
            ]),
            slots.default?.(),
            slots.footer ? h('div', { class: 'mt-4' }, slots.footer()) : null
          ])
        ])
      : null
  }
...
const iconPaths: Record<string, string> = {
  exclamationCircle: 'M12 9v3.75m0 3.75h.007v.008H12V16.5Zm0-13.5a9 9 0 1 0 0 18 9 9 0 0 0 0-18Z',
  infoCircle: 'M11 10h2v7h-2v-7Zm0-3h2v2h-2V7Zm1-5a10 10 0 1 0 0 20 10 10 0 0 0 0-20Z'
}
...
const Icon = defineComponent({
  ...
  setup(iconProps) {
    return () => h('svg', { class: iconProps.size === 'sm' ? 'h-4 w-4' : 'h-5 w-5', viewBox: '0 0 24 24', fill: 'currentColor', 'aria-hidden': 'true' }, [
      h('path', { d: iconPaths[iconProps.name] || iconPaths.infoCircle })
    ])
  }
})
