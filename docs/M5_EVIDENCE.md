...
- The recorder tracks QPS, uptime, total requests, success/error counts, per-account hits/errors, cooldown counts, quota switch counts, recent routing decisions, and a bounded recent-error ring buffer.
...
- Gateway request flow now records successful upstream calls, upstream HTTP errors, local parse/routing/proxy/upstream failures, routing decisions, and cooldown events.
